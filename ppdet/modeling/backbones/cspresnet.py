...
class RepVggBlock(nn.Layer):
    ...
    def forward(self, x):
        if hasattr(self, 'conv'):#查看实例对象中是否存在名为conv的属性
            y = self.conv(x)
        else:
            if self.alpha:
    ...
    def convert_to_deploy(self): #将模型转换为部署模式 即去除多余的参数 设置权重和偏执的值
    ...
    def get_equivalent_kernel_bias(self): #将两个卷积层的卷积核和偏执进行融合获得等效的卷积核和偏执
    ...
    def _pad_1x1_to_3x3_tensor(self, kernel1x1):
        if kernel1x1 is None:
            ...
            return nn.functional.pad(kernel1x1, [1, 1, 1, 1]) #将1*1大小的张量进行补零操作扩充为3*3大小的张量

    def _fuse_bn_tensor(self, branch): #卷积层和批归一化层进行融合，得到融合后的卷积核和偏置，并返回它们的乘积
        if branch is None:
            return 0, 0
        kernel = branch.conv.weight
        running_mean = branch.bn._mean
        running_var = branch.bn._variance  #方差
        gamma = branch.bn.weight1 #尺度参数
        beta = branch.bn.bias1    #偏移参数
        eps = branch.bn._epsilon  #用于数值的稳定性
