...
class DETRLoss(nn.Layer):
    ...
    def __init__(self,
        ...
        self.mdpiou_loss = GIoULoss()  #GIoULoss
    ...
    def _get_loss_bbox(self, boxes, gt_bbox, match_indices, num_gts,
        ...
        loss[name_giou] = self.mdpiou_loss(
    ...
    def forward(self,
        ...
        self.distill_pairs = [boxes, logits, gt_bbox, gt_class]
        ...
        total_loss = self._get_prediction_loss(
            ...
            gt_bbox,
            gt_class,
            masks=masks[-1] if masks is not None else None,  #为None
            gt_mask=gt_mask,  #为None
            postfix=postfix,  #为 “”
            dn_match_indices=dn_match_indices,  #因为不涉及噪声 所以 此处应该也为None
            num_gts=num_gts,
            gt_score=gt_score if gt_score is not None else None)  #也为None
