...
class RTDETRTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        # decoder embedding
        self.learnt_init_query = learnt_init_query   #解码器编码
        ...
        # decoder head
        self.dec_score_head = nn.LayerList([        #这是分类头
            nn.Linear(hidden_dim, num_classes)
            for _ in range(num_decoder_layers)
        ])
        self.dec_bbox_head = nn.LayerList([      #这是回归头
            ...
            self.input_proj.append(
                nn.Sequential(
                    ('conv', nn.Conv2D(
                        ...
                        self.hidden_dim,
                        kernel_size=1,
                        bias_attr=False)), ('norm', nn.BatchNorm2D(   #此处对norm 进行了重命名为norm1 这是因为在加载poolformer权重的时候出现了权重不匹配的问题
                            self.hidden_dim,
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        # decoder
        outputs,out_bboxes, out_logits = self.decoder(
            target,
        ...
        self.student_object = [outputs,feats,out_bboxes, out_logits]
