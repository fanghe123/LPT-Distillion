...
class TransformerDecoderLayer1(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
...
class TransformerDecoder(nn.Layer):
    ...
    def forward(self,
        ...
        return outputs,paddle.stack(dec_out_bboxes), paddle.stack(dec_out_logits)


@register
class RTDETRTransformer1(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eval_idx=-1,
                 eps=1e-2):
        super(RTDETRTransformer1, self).__init__()
        ...
        # Transformer module
        decoder_layer = TransformerDecoderLayer1(
        ...
        if learnt_init_query:
           self.tgt_embed = nn.Embedding(num_queries, hidden_dim)    #它就是query的编码 self.query_embedding
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None, is_teacher=False):
        ...
        # decoder
        outputs,out_bboxes, out_logits = self.decoder(   #outputs是教师网络所有阶段的的query
            target,
        ...
        self.teacher_object=[outputs,out_bboxes, out_logits,self.query_embed,feats]
